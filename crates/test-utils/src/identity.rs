...
impl DidIdentity {
    pub async fn new(owner: &str) -> Self {
        ...
        let url = format!("https://credibil.io/{owner}"); //, uuid::Uuid::new_v4());
        ...
        blockstore.put(owner, "DID", &url, &doc_bytes).await.expect("should put");
    ...
    async fn get_doc(&self, url: &str) -> Result<Document> {
        ...
        let Some(doc_bytes) = self.blockstore.get(owner, "DID", url).await? else {
